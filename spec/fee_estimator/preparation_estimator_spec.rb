...
describe FeeEstimator::PreparationEstimator do
  ...
  Review = Struct.new(:size, :unit, :page_count, :duration, :pages_per_unit, :pages_requiring_redaction, :pages_requiring_redaction_percenatage)
  sample_review = Review.new(200, :page, 300, 30, 1.5, 40, 13)
  actual_review = Review.new(1000, :page, 1000)
  subject {FeeEstimator::PreparationEstimator.new(sample_review, actual_review)}
  ...
  describe :estimate do
    it "returns a search fee estimate" do
      subject.estimate.must_equal 40.00
    end
  end
end
